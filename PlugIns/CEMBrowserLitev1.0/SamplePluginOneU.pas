...
type
  TSampleUILPlugin = class(TJvPlugin)
    ...
    procedure uilPlugin1Commands1Execute(Sender: TObject);
		...
		procedure uilPlugin1Commands3Execute(Sender: TObject);
		function GetModTitle(ThisPageLines:TStringList): String;
...
function TSampleUILPlugin.GetModTitle(ThisPageLines: TStringList): String;
begin

result := 'The mod title from plugint';
...
procedure TSampleUILPlugin.uilPlugin1Commands3Execute(Sender: TObject);
var
	x:TStringList;
begin
	x:= TStringList.Create;

	HostApplication.MainForm.Caption :=  'xxxx';
	ShowMessage(GetModTitle(x)+'XXXXXX');
	FreeAndNil(x);
end;
