...
procedure TFrmMain1.Button_1Click(Sender: TObject);
begin
...
procedure TFrmMain1.Button_CancelSettingsClick(Sender: TObject);
begin
SetUpUI;
end;

procedure TFrmMain1.Button_SaveSettingsClick(Sender: TObject);
begin
SaveSettings();
end;

procedure TFrmMain1.Button_ValidateFoldersClick(Sender: TObject);
begin
...
procedure TFrmMain1.ValidateGameFilesAndFolders();
begin
...
procedure TFrmMain1.ScreenMsg(ThisType:smType;ThisMsg:String);
begin
...
procedure TFrmMain1.Chromium1AfterCreated(Sender: TObject;
	const browser: ICefBrowser);
begin
...
procedure TFrmMain1.Chromium1BeforeClose(Sender: TObject;
  const browser: ICefBrowser);
begin
...
procedure TFrmMain1.Chromium1BeforeResourceLoad(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame;
  const request: ICefRequest; const callback: ICefRequestCallback;
	out Result: TCefReturnValue);
begin
...
procedure TFrmMain1.Chromium1Close(Sender: TObject;
...
procedure TFrmMain1.Chromium1CookiesFlushed(Sender: TObject);
begin
  PostMessage(Handle, MINIBROWSER_COOKIESFLUSHED, 0, 0);
end;

procedure TFrmMain1.Chromium1LoadEnd(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame;
...
procedure TFrmMain1.Chromium1LoadError(Sender: TObject;
...
procedure TFrmMain1.Chromium1LoadingProgressChange(Sender: TObject;
  const browser: ICefBrowser; const progress: Double);
begin
...
procedure TFrmMain1.Chromium1LoadingStateChange(Sender: TObject;
...
procedure TFrmMain1.Chromium1TitleChange(Sender: TObject;
  const browser: ICefBrowser; const title: ustring);
begin
...
procedure TFrmMain1.CustomValidator_modlistfileValidate(Sender: TObject;
  ValueToValidate: Variant; var Valid: Boolean);
begin
Valid := DirectoryExists(FileEdit_modlisttxt.Text  );
end;

procedure TFrmMain1.DBTableView_1CellClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TFrmMain1.ShowSteamModPage();
var
	thisurl: string;
	thismodid:string;
...
procedure TFrmMain1.Slider_BrowserZoomChange(Sender: TObject);
begin
...
procedure TFrmMain1.SaveSettings();
begin
...
{$REGION 'Log'}
  ...
  procedure TFrmMain1.Log(ThisEvent: TLogEventID; ThisMsg: String);
  begin
  varcoded.Log(ThisEvent,ThisMSg);
  end;
  
  procedure TFrmMain1.Log(ThisMsg: String);
  begin
	...
	function TFrmMain1.SetUpLog(): Boolean;
	begin
...
{$ENDREGION}
procedure TFrmMain1.OfficeComboBox_LanguagesChange(Sender: TObject);
begin

siLang_Main.ActiveLanguage := OfficeComboBox_Languages.ItemIndex;
end;

procedure TFrmMain1.OfficeComboBox_SkinChange(Sender: TObject);
begin
SkinData_1.SkinIndex:= OfficeComboBox_Skin.ItemIndex-1;
end;

procedure TFrmMain1.SetupUI;
var
	ThisIndex: smallint;
begin
...
procedure TFrmMain1.Timer_BrowserSetUpTimer(Sender: TObject);
begin
...
procedure TFrmMain1.Timer_StartUpTimer(Sender: TObject);
begin
...
function TFrmMain1.BeginSession(): Boolean;
var
	ThisResult: Boolean;
	db: string;
begin
...
begin
	...
	DM:= TDataModule1.Create(FrmMain1);
...
procedure TFrmMain1.LoadDefaultWebPage(IsActive: Boolean);
var
	TempString:String;
...
procedure TFrmMain1.SetupBrowser();
